...
source = o3d.io.read_point_cloud('task2b_data/kitti_frame1.pcd')
target = o3d.io.read_point_cloud('task2b_data/kitti_frame2.pcd')
...
class ICP:
    ...
    def icp(self, MAX_ITER: int, tol: float) -> np.ndarray:
        ...
        np.save(
            f"task2b_transformations/transformation_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{i}_{tol}.npy", transformation)
