...
class KeyboardPlayer(Player):
    def __init__(self):
        ...
        self.maze_img = cv2.imread(
            'maze_solver\\maze.jpg', cv2.IMREAD_GRAYSCALE)
    ...
    def find_closest_match(self, target_features):
        distances = euclidean_distances(
            [target_features], self.stored_features)[0]
        ...
        try:
            ...
            if self.goal is None and image_number is not None:
                ...
                if image_number is not None:
                    ...
                    print(
                        f"Error: Could not extract image number from path: {matching_path}")
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key in self.keymap:
                    ...
                    self.find_matching_image(self.fpv)
                    maze_solver_obj = maze_solver(src=(int(self.loc_cod[self.curr][0]), int(
                        self.loc_cod[self.curr][1])), dst=self.goal_loc, img=self.maze_img)
                    p = maze_solver_obj.find_shortest_path()
                    maze_solver_obj.drawPath(path=p, thickness=2)
                    cv2.imshow('Maze_Solve', maze_solver_obj.img)
                    cv2.waitKey(1)
    ...
    def show_target_images(self):
        ...
        if self.goal is None:
            ...
            start_point = (140, 260)
            self.goal_loc = (int(self.loc_cod[self.goal][0]), int(
                self.loc_cod[self.goal][1]))  # (30,24) # TBD Based on indexing code
            # end_point=(30,24)
            print(self.goal_loc)
            maze_solver_obj = maze_solver(
                src=start_point, dst=self.goal_loc, img=self.maze_img)
            p = maze_solver_obj.find_shortest_path()
            maze_solver_obj.drawPath(path=p, thickness=2)
            cv2.imshow('Maze_Solve', maze_solver_obj.img)
            cv2.waitKey(1)
        ...
        cv2.putText(concat_img, f'Front View {str(self.goal)}',
                    (h_offset, w_offset), font, size, color, stroke, line)
